...
public class Calculator {
    ...
    public NormalCoordinates calculate(CartesianCoordinates coordinates){
      ...
      //  if (px!=0) normalCoordinates.angle1 = atan (py/px);
        if (px!=0) normalCoordinates.angle1 = atan (px/py);
        else normalCoordinates.angle1= Math.PI/2;

        if (pr-L1 >= 0) normalCoordinates.angle2 = alfa + beta;
        else normalCoordinates.angle2 = alfa - (Math.PI - beta);
